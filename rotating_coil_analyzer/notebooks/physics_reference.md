...
### Segment labelling swap (ALL campaigns up to 2026-02-25)

**IMPORTANT: In ALL MBB campaigns up to 2026-02-25, the DAQ segment labels
were swapped relative to the physical coil positions.** What the DAQ called
"CS" was physically the NCS side, and vice versa.

For pre-2Hz campaigns (2025-12-12, 2026-02-06), both segments were inside
the magnet body, so the swap had no practical impact on the analysis
(both segments produced similar TF values).  The swap was only discovered
in the 2026-02-25 2Hz campaign where one segment is in the fringe field.

**Two naming layers — always be explicit:**

| Layer | "CS" means | "NCS" means |
|-------|-----------|-------------|
| DAQ file labels (raw files, output CSVs) | Physical NCS segment | Physical CS segment |
| Physical segments (magnet reality) | Connection side | Non-connection side |

From the 2026-02-25 2Hz campaign at 4815 A:

| DAQ label | Physical segment | Role | B1 (T) | TF (T/kA) |
|-----------|-----------------|------|--------|-----------|
| CS | NCS (non-connection) | **Body** | -1.782 | 0.370 |
| NCS | CS (connection) | **Fringe** | -0.291 | 0.065 |

The body TF (0.370 T/kA) matches NMR (116 mT at 301 A → TF = 0.385 T/kA).
The fringe TF (0.065 T/kA) is ~6× smaller — characteristic of the field
at the magnet end, mostly outside the yoke.

**Output CSVs and generator configs use DAQ file labels** (because the raw
measurement filenames contain the DAQ labels). The `is_fringe` flag in the
generator config describes the data in the file, not the physical segment:
`CS is_fringe=False` (body data), `NCS is_fringe=True` (fringe data).

**Fix at source:** From the 4 Hz campaign (2026-03-05) onwards, the DAQ
channel assignments are corrected so labels match physical positions.

**Other actions taken:** Notebook directory renamed from
`2026-02-06_NCS_supercycle` to `2026-02-06_CS_supercycle`.  2025-12-12
standalone notebooks swapped (CS_harmonics ↔ NCS_harmonics).

### Comparison with NMR reference values

NMR measures the field at a single point at the magnet centre.  Expected
comparison (body segment = DAQ "CS" file for the 2Hz campaign):

| Current | NMR (centre) | Coil body segment (avg over 0.47 m) | Ratio |
|---------|-------------|-------------------------------------|-------|
...
## 5. Integrated Field from Two Short Coil Segments

### The problem

The rotating coil is 0.47 m long, but the MBB magnet is 6.26 m.  The coil
measures the field averaged over its length, not the total integrated field
that the beam sees.  With two coil segments (body + fringe) positioned at one
end of the magnet, we need to reconstruct the full ∫B·dz.

### What the coil measures

Each segment measures the field averaged over its 0.47 m span:

```
B_coil = (1/L_coil) × ∫ B(z) dz    (over the coil length)
```

So `B_coil × L_coil` = field integral over that 0.47 m slice.

The same applies to every harmonic coefficient C_n: the coil gives the
**length-averaged** C_n, not a point value.

### Physical setup and partition
...
We partition the full magnet into three non-overlapping regions:

| Region | Length | Average field |
|--------|--------|--------------|
| Left fringe | L_coil | B_fringe (measured) |
| Body exclusive | L_yoke − 2f·L_coil | B_body (measured) |
| Right fringe | L_coil | B_fringe (by symmetry) |

where `f` is the overlap fraction (default 0 = fringe coil entirely outside).
...
```
B_int = B_body × L_body_excl + 2 × B_fringe × L_coil     [T·m]
```

**Magnetic length** (length of a uniform magnet with the same B_int):

```
L_mag = B_int / B_body                                     [m]
```

**Integrated harmonics** (must work in Tesla, not units):

```
C_n = b_n × B1 / 10000                 (per segment, convert units → T)
∫C_n dz = C_n_body × L_body + 2 × C_n_fringe × L_coil    (integrate in T·m)
b_n_int = (∫C_n dz) / B_int × 10000    (re-normalize to units)
```

Key point: you cannot average b_n values directly because b_n = C_n/C_1×10⁴
is a ratio, and the two segments have different C_1.

### Assumptions

1. **Uniform body field**: B(z) = B_body for all z inside the yoke.
   Valid for the MBB: long dipole, well-shimmed, confirmed by NMR comparison.
   This is an extrapolation by ×13 (0.47 → 6.26 m).

2. **Symmetric fringes**: Both ends have identical fringe profiles.
   Only one end is measured.  Reasonable for a symmetric yoke design.

3. **Complete fringe capture**: The 0.47 m fringe coil captures the full
   field drop-off.  If significant field extends further, B_int is
   underestimated.

4. **No overlap** (default): The fringe coil sits entirely outside the yoke.
   The geometry sensitivity sweep (overlap 0.0–0.5) in the notebook quantifies
   the impact: ΔL_mag ≈ ±0.04 m.

### Sanity checks

- **L_mag > L_yoke**: Expected because fringe extends beyond iron.
  Result: L_mag = 6.42 m = L_yoke + 0.16 m (80 mm per end).
- **L_mag decreases at flat-top**: Saturation reduces fringe relative to body.
  Result: 6.419 m (injection) → 6.414 m (4815 A).  Tiny effect.
- **Integrated b_n ≈ body b_n**: Body dominates (87% of length, 6× higher B1).
  Result: b3_int ≈ −0.09 units vs b3_body ≈ −0.23 units.  Fringe shifts
  it toward zero (fringe b3 is positive).
